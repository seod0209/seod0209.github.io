...
Google AdSense를 등록하던 도중 ads.txt를 찾을수 없다는 알림이 보였다. 그래서 ads.txt를 어떻게 넣어야 할까 고민을 하던중 여기저기 검색해봤는데
...
띠용..? 너무 간단한 방법이긴 한데.. 내가 숨겼던 id나 개인 정보가 ads.txt 내에 그대로 적혀있는데 이러면 숨길 필요가 없는거 아닌가. 근데 아무리 생각해도 숨겨야할것같았다 나의 소듕한 정보.


> 일단 내 레포는 public이여서 누구나 해당파일에 접근해서 볼수 있기 때문

> ⚠️ (팩트체크) ads.txt는 광고 크롤러가 도메인 루트(`/ads.txt`)에서 읽을 수 있어야 정상 동작하는 **공개 파일**이며, 그 안의 publisher ID도 원래 공개용 식별자다. 따라서 레포가 public이든 아니든 ads.txt를 숨겨도 실제 보안상 이점은 없다. 아래에서 설명하는 CI/CD 동적 생성 기법 자체는 유용하지만, 예시 대상은 ads.txt가 아니라 API 키나 서비스 계정 JSON처럼 **진짜 숨겨야 하는 비밀 값**으로 바꿔 생각하는 것이 맞다.
...
- 애플리케이션 코드와 함께 ads.txt 파일을 직접 배포 패키지에 포함시키는 방법
...
> 다만 GitHub의 Secrets 로그 마스킹은 **보장이 아니라 best-effort**다. 값이 변형되어 출력되거나(예: base64 인코딩) 여러 줄로 쪼개지면 마스킹을 우회할 수 있으므로, 로그에 비밀 값을 절대 출력하지 않도록 스크립트 차원에서 주의해야 한다.
...
## CI/CD 스크립트에서 파일 생성:
 여기서는 GitHub Actions를 예로 들어 CI/CD 파이프라인에서 ads.txt 파일을 처리하는 방법을 설명하겠습니다. 배포 프로세스에서 ads.txt 파일을 동적으로 생성하거나 환경 변수를 사용하여 포함시키는 방법을 코드 예제이다.

1. 배포 스크립트에서 파일 동적으로 생성하기
GitHub Actions를 사용하여 배포 과정에서 ads.txt 파일을 동적으로 생성하는 예제입니다. 이 방법은 민감한 데이터를 CI/CD 환경에서 안전하게 처리할 수 있게 해줍니다.
  ...
  build:
    ...
    steps:
      ...
      - name: Create ads.txt
        ...
        run: echo "$SENSITIVE_DATA" > ads.txt

      - name: Deploy
        run: |
          # 예를 들어, 배포 스크립트를 호출하거나 서버에 파일을 전송하는 명령어
          scp ads.txt user@server:/path/to/deploy/
          # 추가 배포 작업
```
이 설정에서 SENSITIVE_DATA는 GitHub Secrets에 저장된 민감한 데이터이다.

> ⚠️ 위 예제의 `scp` 단계는 그대로 두면 실패한다. GitHub Actions 러너에는 서버 접속용 SSH 개인키와 `known_hosts`가 없기 때문이다. 실제로는 `webfactory/ssh-agent` 같은 액션으로 `secrets`에 넣어 둔 SSH 키를 등록하고 `ssh-keyscan`으로 `known_hosts`를 설정하는 단계를 먼저 넣어야 한다.
