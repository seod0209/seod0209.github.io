...
여기서 한 가지 의문이 생긴다. 실제 실행 흐름에서는 바깥의 세부사항(DB, 프레임워크)이 호출되는데 어떻게 안쪽이 바깥을 알지 못할 수 있을까? 이는 의존성 역전 원칙(DIP, Dependency Inversion Principle)으로 해결된다. 안쪽 계층이 인터페이스(추상)를 정의하고 바깥 계층이 이를 구현하도록 하여, 소스코드 상의 의존성 방향을 실제 제어 흐름과 반대로 뒤집는 것이다. 덕분에 안쪽은 추상에만 의존하고, 구체적인 세부사항은 바깥에서 얼마든지 갈아 끼울 수 있다.

크게 안쪽에는 도메인의 영역인 entity, use case 그리고 가장 바깥층(가장 낮은 수준)에는 Infrastructure, 그 둘 사이를 이어주는 adaptor가 있다.
