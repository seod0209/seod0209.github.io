...
리덕스 스토어에는 직렬화 가능한 값을 저장하는 것이 원칙이라, 클래스 인스턴스는 그대로 저장하지 않는 것이 권장된다. 때문에 매번 `classToPlain`으로 class를 json객체로 풀어서 넣어주고, 리덕스에서 나온 데이터는 다시 class로 만들어주어야 했다.
