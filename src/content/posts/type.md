...
JavaScript의 모든 number 값은 int, float, double 같은 구분 없이 IEEE 754 배정밀도(double precision) 부동소수점으로 표현된다.
따라서 정수(-1, 0, 1)와 실수(0.57, 167.6)를 별도의 타입으로 나누지 않고 하나의 number 타입으로 다룬다.
(큰 정수를 안전하게 다루기 위한 별도의 BigInt 타입이 있지만, int/float/double 같은 구분은 존재하지 않는다.)

> 우리가 주로 사용하는 10진수는 10을 기준으로 한다(숫자 0~9을 쓴다는 뜻). 하지만 자바스크립트는 10진수 이외의 다른 숫자 타입을 지원한다.
