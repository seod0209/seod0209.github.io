...
회사는 글로벌 시장을 대상으로 하고 있으며, 특히 인도와 아랍 국가들을 타겟 Meta광고를 하고 있다. 이로 인해 앱 다운로드 수는 증가했지만, 설치 후 바로 삭제되는 현상이 발생하고 있다. 또한 Sentry에서 가장 많이 관측되는 에러는 Network에러 였다. Network 에러가 잦다는 사실 자체가 "용량이 큰 앱을 피한다"를 직접 증명하는 것은 아니지만(상관관계일 뿐 인과로 단정하긴 어렵다), 적어도 타겟 시장의 네트워크 환경이 열악하다는 정황으로는 볼 수 있으며 용량 최적화의 필요성을 뒷받침한다.
...
3. ProGuard(R8)를 사용한 코드 난독화와 최적화. 사용되지 않는 코드를 제거하고 클래스 및 메서드 이름을 난독화하여 APK 파일의 크기를 줄일 수 있다.

   > 💡 현재 Android Gradle Plugin에서는 ProGuard 대신 R8이 기본 축소/난독화 도구다. 기존 `proguard-rules.pro` 규칙은 R8에서 그대로 호환된다.
...
2. **코드 난독화**: ~~Xcode는 코드 난독화 기능을 내장하고 있어...~~ → 이는 사실이 아니다. **Xcode에는 코드 난독화 기능이 없다.** Xcode가 릴리즈 빌드에서 하는 심볼 스트리핑(symbol stripping)이나 dSYM 분리는 디버그 심볼을 제거·분리하는 것일 뿐 난독화가 아니다. iOS에서 실제 코드 난독화를 하려면 SwiftShield 같은 서드파티 도구를 별도로 도입해야 한다.
...
- 프로그레시브 웹 앱 (PWA): PWA를 사용하여 사용자가 앱을 다운로드하지 않고도 웹 브라우저에서 앱을 실행할 수 있다. 이를 통해 앱을 설치하는 과정을 생략할 수 있으며, 이는 앱을 다운로드하는 도중 이탈하는 사용자들을 막을 수 있다고 본다. 다만 "네트워크에 상관없이 사용할 수 있다"는 것은 정확하지 않다. **최초 로딩과 아직 캐시되지 않은 리소스는 네트워크가 필요**하며, 오프라인 동작은 Service Worker가 미리 캐시해 둔 범위로 한정된다. 오히려 네트워크가 열악한 시장이 타겟이라면 이 캐싱 전략을 신중히 설계해야 한다.
...
## 출처
- [Shrinking APKs, growing installs — Google Play](https://medium.com/googleplaydev/shrinking-apks-growing-installs-5d3fcba23ce2)
