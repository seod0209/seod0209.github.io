...
Fetch에는 일반적인 오브젝트로 Request 와 Response 가 포함되어 있다. 이것들은 service worker이나 Cache API같이 **Response와 Request객체를 다루는 API**나 **독자적으로 리스폰스**를 발생시키는 경우에도 사용 가능하다.
...
- 백에드에서 보내준 API를 대상으로 새로운 포스트를 생성하기 위해서 fetch() 함수를 사용해보았다. method 옵션을 POST로 지정해주고, headers 옵션을 통해 JSON 포맷을 사용한다고 알려줘야 하며, 요청 전문을 JSON 포맷으로 직렬화하여 가장 중요한 body 옵션에 설정해준다.

#### 백엔드의 토큰값을 가지고 true/false에 따라 다른 알림창을 띄워보았다.
