...
image: "https://velog.velcdn.com/images/seod0209/post/87b11bb2-59f8-448e-8808-75ad77ec2313/image.png"
---
...
- 기술스택
    - 언어: Typescript
    - 프레임워크: React Native, React, React Query , Recoil , Emotion , Next.js
    - CI/CD: Firebase Distribution , GitHub Actions을 통한 CodePush 배포로 추가 심사없이 앱 업데이트 가능
...
- 원하는 기능을 구현하기에 3개월이라는 기한은 턱없이 부족한 시간이었기에, beside 활동이 끝난 후에 추가적으로 3개월을 더 소요해서 mvp 기능들을 완료하였다.
...
## 프로젝트 후기
 그룹, 약속, 위치공유, 로그인 주요기능을 4가지만 가지고 가기에 시간과 인력이 넉넉할거라고 생각한건 정말 단단히 착각이였다..
...
> 여담이지만 다들 본업이 없는사람들처럼 사이드 프로젝트에 참여했다ㅋㅋㅋ 그래서 짧은 기간에 다른 팀에 비해 고퀄의 결과물이 나오지 않았나 싶다.
...
####  :: 소외된 AOS
 ...
 - 각 OS가 추구하는 정책이 다른것도 유의 했어야 한다. (예를들면 소셜로그인만 제공할 경우 iOS는 애플로그인 제공이 사실상 필수다. AOS도 비회원 로그인이 필수라고 당시엔 단정했었는데, 다시 찾아보니 그런 스토어 정책 근거는 확인하지 못했다 — 근거 없이 정책을 단정한 것도 반성할 점.)
