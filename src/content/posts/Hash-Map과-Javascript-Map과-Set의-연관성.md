...
tags: ["javascript", "map", "set", "hashmap"]
---
...
- 해시 기반의 자료구조:
  ECMAScript 명세는 평균적으로 서브리니어(sublinear) 시간의 접근만 요구할 뿐, 해시 함수 사용을 강제하지 않음. V8 등 실제 엔진은 해시 테이블로 구현하지만 이는 명세가 요구하는 것은 아님. 이를 통해 요소에 대한 빠른 검색과 접근이 가능.
...
- 효율적인 메모리 관리:
  V8은 해시 테이블로 내부 저장 공간을 효율적으로 관리하지만, 이는 명세가 요구하는 것은 아니며 해시 충돌도 일반적으로 발생
...
// Set.prototype 의 Symbol.iterator 메서드를 상속받는 이터러블
...
> (참고: intersection, union, difference 등은 ES2025부터 Set의 표준 내장 메서드로 추가됨. 따라서 아래처럼 Set.prototype에 직접 정의하면 네이티브 구현을 덮어쓰게 되므로 주의.)
...
- 키와 값의 쌍을 연결하여 저장. 삽입한 순서가 보장됨(반복 시 삽입 순서대로 순회).
