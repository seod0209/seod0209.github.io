...
> 💡 이 글은 Pages Router(`pages/` 디렉터리) 기준입니다. `next.config.js`의 `i18n` 옵션과 `next-i18next`는 App Router에서는 지원되지 않으며, App Router에서는 `next-intl` 등을 사용해 직접 라우팅/미들웨어를 구성하는 방식이 권장됩니다.
