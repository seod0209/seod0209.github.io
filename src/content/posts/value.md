...
> (정정: alert는 예약어(reserved word)가 아니라 window 객체의 메서드(window.alert)다. if·for·class·return 같은 것이 진짜 예약어이며, 예약어가 아닌 alert·console.log 등은 변수 이름으로 덮어쓸 수는 있으나 권장되지 않는다.)
...
> - 대소문자 구분 - 변수이름, 함수이름, 연산자 모두 대소문자를 구분합니다. 따라서 myName과 MyName은 다른 변수입니다.
...
2. const: 재할당(바인딩 변경) 불가능. 즉 다른 값을 다시 할당할 수 없을 뿐, 객체·배열을 할당한 경우 그 내부 프로퍼티/요소는 변경할 수 있다(불변이 아니라 재할당만 막는다). 재할당할 일이 없는 경우 사용.
