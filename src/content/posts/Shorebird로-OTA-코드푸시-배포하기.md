...
> ⚠️ 다만 OTA로 뭐든 갈아끼울 수 있는 건 아니다. Shorebird는 **Dart 코드 패치**가 중심이고, 네이티브 코드/플러그인/에셋을 바꾸거나 앱의 핵심 기능·목적을 바꾸는 변경은 스토어 심사 정책상 OTA로 우회하면 안 된다. (Apple App Store 심사 지침, Google Play 정책 모두 심사 없이 앱 동작을 실질적으로 바꾸는 걸 제한한다.) OTA는 어디까지나 "이미 심사받은 앱의 급한 버그 수정" 용도로 선을 긋고 쓰는 게 안전하다.
